...
class AutoUpdate
{
    ...
    public function getVersionsToUpdateArray()
    {
        if (count($this->updates) > 0) {
            return $this->updates;
        }
    ...
    public function update($simulateInstall = true, $deleteDownload = true)
    {
        ...
        foreach ($this->updates as $update) {
            ...
            if ( ! is_file($updateFile)) {
                ...
                ## 判断文件大小
                ## 如果超过 100M 则提示手动下载
                ## var_dump($res['Content-Length']);
                $res       = get_headers($update['url'], true);
                $file_size = isset($res['Content-Length']) ? $res['Content-Length'] : 0; // 可能配置错误，文件不存在
                //echo $file_size;
                if ($file_size > 100 * 1024 * 1024) {
                    $message = '需要下载的文件太大，请手动下载!';
                    $this->log->addCritical(sprintf($message));
    ...
    private function check_memory()
    {
        ...
        $ini_memory_int = intval($ini_memory);
        $ini_memory_str = trim($ini_memory, $ini_memory_int);
        $ini_memory_str = strtolower(substr($ini_memory_str, 0, 1));
        switch ($ini_memory_str) {
            case 't':
                $result = $ini_memory_int * 1024 * 1024 * 1024 * 1024;
                break;
            case 'g':
                $result = $ini_memory_int * 1024 * 1024 * 1024;
                break;
            case 'm':
                $result = $ini_memory_int * 1024 * 1024;
                break;
            case 'k':
                $result = $ini_memory_int * 1024;
                break;
            default:
                $result = $ini_memory_int;
                break;
        }
